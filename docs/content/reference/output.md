...
describes what users get. Pick a format with `-o`, or let gc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gc <command> -o table   # aligned columns for reading
gc <command> -o jsonl   # one JSON object per line, for piping
gc <command> -o json    # a single JSON array
gc <command> -o csv     # spreadsheet friendly
gc <command> -o tsv     # tab-separated
gc <command> -o url     # just the URL column
gc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gc <command> --fields id,title,url
```
...
```bash
gc <command>            # a table, because this is a terminal
gc <command> | wc -l    # JSONL, because this is a pipe
```
