// Package cli assembles the gc command tree from the globalcomix
...
// NewApp assembles the kit application from the globalcomix domain. The
...
//
// To add a command, declare it in globalcomix/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := globalcomix.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(globalcomix.Domain{}).Register(app)
